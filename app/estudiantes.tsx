...
export default function EstudiantesScreen() {
  ...
  const { programaPredefinido } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (programaPredefinido) {
      ...
      const codigoP = Array.isArray(programaPredefinido) ? programaPredefinido[0] : programaPredefinido;
      setProgramaCod(codigoP);
    }
  }, [programaPredefinido]);
  ...
  const cargarEstudiantes = (termino = '') => {
    let result;
    if (termino) {
      result = db.getAllSync<Estudiante>('SELECT * FROM estudiantes WHERE codigo LIKE ? OR nombre LIKE ?', [`%${termino}%`, `%${termino}%`]);
    } else {
  ...
  const validarCampos = () => {
    ...
    if (!codigo.trim() || !nombre.trim() || !email.trim() || !programaCod.trim()) { Alert.alert('Error', 'Todos los campos son obligatorios'); return false; }
    if (!emailRegex.test(email.trim())) { Alert.alert('Error', 'Por favor ingresa un correo electrónico válido'); return false; }
    if (codigo.length > 4 || programaCod.length > 4) { Alert.alert('Error', 'Los códigos no pueden superar los 4 caracteres.'); return false; }
    return true;
  };
  ...
  const guardarEstudiante = () => {
    ...
    try {
      if (editando) {
        ...
        db.runSync('INSERT INTO estudiantes (codigo, nombre, email, programa_cod) VALUES (?, ?, ?, ?)', [codigo.trim(), nombre.trim(), email.trim(), programaCod.trim()]);
      }
      limpiarFormulario();
      cargarEstudiantes();
    } catch (error) {
  ...
  const editarEstudiante = (est: Estudiante) => {
    setCodigo(est.codigo); setNombre(est.nombre); setEmail(est.email); setProgramaCod(est.programa_cod); setEditando(true);
  };

  const eliminarEstudiante = (cod: string) => {
    Alert.alert('Confirmar', '¿Eliminar este estudiante?', [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Eliminar', style: 'destructive', onPress: () => { db.runSync('DELETE FROM estudiantes WHERE codigo = ?', [cod]); cargarEstudiantes(); } }
    ]);
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={[styles.input, (editando || !!programaPredefinido) && styles.disabledInput]} placeholder="Ej: P001" value={programaCod} onChangeText={setProgramaCod} maxLength={4} editable={!editando && !programaPredefinido} autoCapitalize="characters" />
          </View>
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <View style={styles.cardActions}>
                <TouchableOpacity style={styles.actionButton} onPress={() => editarEstudiante(item)}><Text style={styles.actionTextEdit}>Editar</Text></TouchableOpacity>
                <TouchableOpacity style={styles.actionButton} onPress={() => eliminarEstudiante(item.codigo)}><Text style={styles.actionTextDelete}>Eliminar</Text></TouchableOpacity>
              </View>
            </View>
          )}
        />
...
const colores = { rojoPrimario: '#C4142B', fondoApp: '#FAFAFA', textoOscuro: '#222222', textoGris: '#555555', bordeGris: '#DCDCDC', blanco: '#FFFFFF', azulInput: '#F0F4F8' };
const styles = StyleSheet.create({
  ...
  textoVolver: { color: colores.textoGris, fontSize: 16, fontWeight: '500' },
  ...
  card: { flexDirection: 'row', backgroundColor: colores.blanco, padding: 16, borderRadius: 8, marginBottom: 12, alignItems: 'center', borderLeftWidth: 4, borderLeftColor: colores.rojoPrimario, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  cardTitle: { fontWeight: 'bold', fontSize: 16, color: colores.textoOscuro, marginBottom: 2 },
  cardSubtitle: { fontSize: 13, color: colores.textoGris, marginBottom: 2 },
